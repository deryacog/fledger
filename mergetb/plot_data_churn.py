...
import sys
import os
from plot_data import get_data, save_plot_directory, plot_latency_components, plot_reliability, plot_incoming_messages, plot_latency, plot_bandwidth, plot_latency_and_bandwidth, plot_reliability_latency, convert_to_milliseconds
...
if __name__ == "__main__":
    ...
    for variable, run in data.items():
        ...
        bar_width = 0.2
        group_spacing = 0.1
        x_indexes = np.arange(len(points_in_time_labels)) * (len(retry_levels) * bar_width + group_spacing)
        ...
        reliability_data = {
            retry: [
                convert_to_milliseconds(run[retry][str(pt)]["loopix_end_to_end_latency_seconds"]) 
                for pt in points_in_time
            ]
            for retry in retry_levels
        }
        ...
        for retry, reliabilities in reliability_data.items():
            reliability = [r for r in reliabilities]
            plt.bar(
                [x + offset for x in x_indexes],
                reliability,
                width=bar_width,
        ...
        plt.title(f"Latency with {variable}", fontsize=16)
        ...
        save_plot_directory(directory)
        plt.savefig(os.path.join(directory, f"{variable}_latency_churn_bar_chart.png"))
        plt.clf()
        plt.close()
        ...
        reliability_data = {
            retry: [
                run[retry][str(pt)]["loopix_total_bandwidth_mb"] 
                for pt in points_in_time
            ]
            for retry in retry_levels
        }
        ...
        plt.title(f"Total Network Usage over 300 seconds with {variable}", fontsize=16)
        plt.ylabel("Network Usage (MB)", fontsize=14)
        plt.xticks([x + bar_width * (len(retry_levels) - 1) / 2 for x in x_indexes], points_in_time_labels, fontsize=12)
        plt.legend(fontsize=12)
        plt.grid(axis='y', linestyle='--', alpha=0.7)

        save_plot_directory(directory)
        plt.savefig(os.path.join(directory, f"{variable}_bandwidth_churn_bar_chart.png"))
        plt.clf()
        plt.close()
